...
export function ColaboradoresEmbalagemProvider({ children }) {
    ...
    const fetchColaboradores = async () => {
        const { data, error } = await supabase.from('colaboradores').select('*').eq('setor', 'embalagem');
        if (error) {
            console.error('Erro ao buscar colaboradores da embalagem:', error);
            return;
        }
    ...
    useEffect(() => {
        ...
        const subscription = supabase.channel('colab-embalagem-channel')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'colaboradores', filter: "setor=eq.embalagem" }, payload => {
                fetchColaboradores();
            })
            .subscribe();
...
export const useColaboradoresEmbalagem = () => useContext(ColaboradoresEmbalagemContext);
