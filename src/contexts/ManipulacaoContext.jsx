...
export function ManipulacaoProvider({ children }) {
    ...
    const fetchItems = async () => {
        const { data, error } = await supabase.from('itens').select('*').eq('setor', 'manipulacao');
        if (error) {
            console.error('Erro ao buscar itens de manipulação:', error);
            return;
        }
    ...
    useEffect(() => {
        ...
        const subscription = supabase.channel('itens-manipulacao-channel')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'itens', filter: "setor=eq.manipulacao" }, payload => {
                fetchItems();
            })
            .subscribe();
    ...
    const addItem = async (itemData) => {
        const newId = 'man-' + Date.now().toString();
        const initialLog = {
            id: 'log-' + Date.now().toString() + Math.random().toString(36).substr(2, 9),
            itemId: newId,
        ...
        if (error) {
            console.error('Erro ao adicionar item de manipulação:', error);
            return;
        }
