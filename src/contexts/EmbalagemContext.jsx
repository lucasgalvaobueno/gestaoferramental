...
export function EmbalagemProvider({ children }) {
    ...
    const fetchItems = async () => {
        const { data, error } = await supabase.from('itens').select('*').eq('setor', 'embalagem');
        if (error) {
    ...
    useEffect(() => {
        ...
        const subscription = supabase.channel('itens-embalagem-channel')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'itens', filter: "setor=eq.embalagem" }, payload => {
                fetchItems();
            })
            .subscribe();
    ...
    const addItem = async (itemData) => {
        const newId = 'emb-' + Date.now().toString();
        const initialLog = {
            id: 'log-' + Date.now().toString() + Math.random().toString(36).substr(2, 9),
            itemId: newId,
        ...
        if (error) {
            console.error('Erro ao adicionar item de embalagem:', error);
            return;
        }
